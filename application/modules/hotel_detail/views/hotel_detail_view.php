...
<div class="hotel_availability">
    ...
    <table class="table-responsive">
        ...
        <?php //print_r($hotel_detail);?>			 
             <tr>
                ...
                <td>
                    ...
                    <input name="numb" id="numb" type="hidden" value="1">                    
                     <input name="room_type" type="hidden" value="Single">
                     ...
                     <input name="ChildrenAges" type="hidden" value="<?=$search_array->childrenAges[0]?>">
                     <input name="TotalRooms" type="hidden" value="1">
                     <input name="TotalRate" type="hidden" value="100">                     
                     <input name="countrycode" type="hidden" value="<?=$search_array->city[1]?>">
                     <input name="city" type="hidden" value="<?=$search_array->city[0]?>">                    
                     <input name="hotel_id" type="hidden" value="<?=$hotel_detail->hotel_id?>">                
             ...
             <tr>
                <td><?php //print_r((string)$room->BookingKey);?>
                    <img src="<?php echo base_url();?>assets/images/1.jpeg"> <h6>Double<?php //if(!empty($room->Type)) { print_r(explode('|',(string)$room->Type)[0]); } else { }; ?></h6><p><?php //print_r(explode('|',(string)$room->RoomDescription)[0])?></p>
                </td>
                <td>
                    AED <?php //echo round($room->TotalRate,2)?>200/night
                </td>
                <!--<td>
                    <select name="no_of_rooms" onchange="set_room<?//$key?>(this.value);"><option>1</option><option>2</option><option>3</option><option>4</option></select>
                </td>-->
                <td>
                    <form id="bookingform2" action="#<?php //echo site_url('pre_booking');?>" onclick="return false;" method="post" >                                        
                    <input name="numb" id="numb" type="hidden" value="2">                    
                     <input name="room_type" type="hidden" value="Double">
                     <input name="SearchSessionId" type="hidden" value="123">   
                     <input name="BookingKey" type="hidden" value="1234567">
                     <input name="Adults" type="hidden" value="<?=$search_array->adults?>">
                     <input name="Children" type="hidden" value="<?=$search_array->childs?>">
                     <input name="ChildrenAges" type="hidden" value="<?=$search_array->childrenAges[0]?>">
                     <input name="TotalRooms" type="hidden" value="1">
                     <input name="TotalRate" type="hidden" value="200">                     
                     <input name="countrycode" type="hidden" value="<?=$search_array->city[1]?>">
                     <input name="city" type="hidden" value="<?=$search_array->city[0]?>">                    
                     <input name="hotel_id" type="hidden" value="<?=$hotel_detail->hotel_id?>">
                    <input name="room_rate" type="hidden" value="200">
                    
                    <input name="key" type="hidden" value="2">
                    <button class="btn btn-danger" id="select_room2" >Select Room</button>
                    </form>
                </td>
            </tr>
             <tr>
                <td><?php //print_r((string)$room->BookingKey);?>
                    <img src="<?php echo base_url();?>assets/images/1.jpeg"> <h6>Large<?php //if(!empty($room->Type)) { print_r(explode('|',(string)$room->Type)[0]); } else { }; ?></h6><p><?php //print_r(explode('|',(string)$room->RoomDescription)[0])?></p>
                </td>
                <td>
                    AED <?php //echo round($room->TotalRate,2)?>400/night
                </td>
                <!--<td>
                    <select name="no_of_rooms" onchange="set_room<?//$key?>(this.value);"><option>1</option><option>2</option><option>3</option><option>4</option></select>
                </td>-->
                <td>
                    <form id="bookingform4" action="#<?php //echo site_url('pre_booking');?>" onclick="return false;" method="post" >                                        
                    <input name="numb" id="numb" type="hidden" value="4">                    
                     <input name="room_type" type="hidden" value="Large">
                     <input name="SearchSessionId" type="hidden" value="123">   
                     <input name="BookingKey" type="hidden" value="12345678">
                     <input name="Adults" type="hidden" value="<?=$search_array->adults?>">
                     <input name="Children" type="hidden" value="<?=$search_array->childs?>">
                     <input name="ChildrenAges" type="hidden" value="<?=$search_array->childrenAges[0]?>">
                     <input name="TotalRooms" type="hidden" value="1">
                     <input name="TotalRate" type="hidden" value="400">                     
                     <input name="countrycode" type="hidden" value="<?=$search_array->city[1]?>">
                     <input name="city" type="hidden" value="<?=$search_array->city[0]?>">                    
                     <input name="hotel_id" type="hidden" value="<?=$hotel_detail->hotel_id?>">
                    <input name="room_rate" type="hidden" value="400">
                    
                    <input name="key" type="hidden" value="4">
                    <button class="btn btn-danger" id="select_room4" >Select Room</button>
                    </form>
                </td>
            </tr>
		<?php }?>        
	...
	<script>
                ...
                $('#select_room2').on('click', function(){
                   //alert('kk');
                    $.ajax({
                        url: '<?php echo site_url('hotel_detail/add_room')?>',
                        type: 'post',
                        //dataType: 'json',
                        data: $('form#bookingform2').serialize(),
                        success: function(response) {
                                  console.log(response);
                                  document.getElementById('cart').innerHTML = response;
                                  return false;
                ...
                $('#select_room4').on('click', function(){
                   //alert('kk');
                    $.ajax({
                        url: '<?php echo site_url('hotel_detail/add_room')?>',
                        type: 'post',
                        //dataType: 'json',
                        data: $('form#bookingform4').serialize(),
                        success: function(response) {
                                  console.log(response);
                                  document.getElementById('cart').innerHTML = response;
                                  return false;
   ...
   var latlng = new google.maps.LatLng(<?php  echo $hotel_detail->latitude; ?>,<?php  echo $hotel_detail->longitude; ?>);
    var map = new google.maps.Map(document.getElementById('map'), {
	  ...
	  var request = {
		location: latlng,
		radius: '500',
		type: ['golf club','bar','pub','restaurant','zoo','icecream','food','store','spa','cafe']
	  };
	...
	function createMarker(place) {
		...
		if(type.indexOf("cafe") >= 0){
			image = '<?php echo base_url();?>/upload/golf.png';
		}else if(type.indexOf("spa") >= 0){			
			image = '<?php echo base_url();?>/upload/zoo.png';
		}else if(type.indexOf("bar") >= 0){			
			image = '<?php echo base_url();?>/upload/bar.png';
		}else if(type.indexOf("restaurant") >= 0 || type.indexOf("food") >= 0){			
			image = '<?php echo base_url();?>/upload/hotel.png';
		}else if(type.indexOf("golf club") >= 0){
			image = '<?php echo base_url();?>/upload/golf.png';
		}else if(type.indexOf("zoo") >= 0){
			image = '<?php echo base_url();?>/upload/zoo.png';
		}else if(type.indexOf("pub") >= 0){
			image = '<?php echo base_url();?>/upload/music.png';
		}else if(type.indexOf("temple") >= 0){
			image = '<?php echo base_url();?>/upload/temple.png';
		}else if(type.indexOf("store") >= 0){
			image = '<?php echo base_url();?>/upload/home.png';
		}else{
			image = '<?php echo base_url();?>/upload/hotel.png';
		}
